...
git fetch origin "$CURRENT_BRANCH" --quiet 2>>"$LOGFILE"
REMOTE=$(git rev-parse --short=7 "origin/$CURRENT_BRANCH" 2>/dev/null)
...
if [ "$LOCAL" != "$REMOTE" ] && [ -n "$REMOTE" ]; then
  ...
  for i in $(seq 1 $MAX_RETRIES); do
    if git pull origin "$CURRENT_BRANCH" --quiet 2>>"$LOGFILE"; then
      ...
      echo "[$DATE] OK    $LOCAL -> $NEW | $NEWMSG" >> "$LOGFILE"
      break
    else
      echo "[$DATE] FAIL  attempt $i/$MAX_RETRIES" >> "$LOGFILE"
