...
按研究领域分组，每组包含：
**重要：同一研究领域的内容必须归入同一板块，不得按来源、期刊或细节主题拆分为多个板块。例如：**
- 不得将"交互式叙事"和"游戏技术哲学"分开设立两个板块 → 应合并为"交互式叙事与游戏技术哲学"
- 不得将"游戏叙事学"按不同期刊拆分为多个板块
- 同一领域的论文（无论来自 arXiv、CrossRef 还是其他来源）必须合并在一起
- 如果某领域内容较少，可合并到最相关的相邻领域板块中

**重要：每个研究领域的论文严格控制在1-2篇，宁缺毋滥。尤其是"游戏化学习""教育应用""教学方法"等教育类栏目，最多1篇即可。如果某领域论文质量普遍不高或与游戏研究核心关联较弱，可直接跳过不展示。优先选择质量最高、最贴近游戏研究本质的论文。**
